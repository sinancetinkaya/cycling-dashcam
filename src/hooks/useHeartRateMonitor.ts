...
export function useHeartRateMonitor(smoothing: number = 3, calibration: number = 0) {
  ...
  const connectToDevice = async (device: any) => {
    ...
    characteristic.addEventListener('characteristicvaluechanged', (event: any) => {
      ...
      while (hrHistoryRef.current.length > currentSmoothing) hrHistoryRef.current.shift();
      const avgHr = Math.round(hrHistoryRef.current.reduce((a, b) => a + b, 0) / hrHistoryRef.current.length);
      
      // Apply calibration
      setHeartRate(avgHr + calibrationRef.current);
    });
  ...
  const connect = useCallback(async (options?: { autoConnectOnly?: boolean; forcePicker?: boolean }) => {
    ...
    try {
      ...
      if (autoConnectOnly && !forcePicker) {
