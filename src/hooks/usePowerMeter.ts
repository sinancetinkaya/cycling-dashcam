...
export function usePowerMeter(smoothing: number = 3, calibration: number = 0) {
  ...
  const connectToDevice = async (device: any) => {
    ...
    characteristic.addEventListener('characteristicvaluechanged', (event: any) => {
      ...
      while (powerHistoryRef.current.length > currentSmoothing) powerHistoryRef.current.shift();
      const avgPower = Math.round(powerHistoryRef.current.reduce((a, b) => a + b, 0) / powerHistoryRef.current.length);
      
      // Apply calibration
      setPower(avgPower + calibrationRef.current);
    });
